...
Micronaut Docs can be imported into IntelliJ IDEA by opening the `build.gradle` file.
...
To get a local development version of Micronaut Docs working, first run the `publishToMavenLocal` task.
...
Micronaut Docs uses [Checkstyle](https://checkstyle.sourceforge.io/) to make sure that all the code follows those standards. The configuration file is defined in `config/checkstyle/checkstyle.xml` and to execute the Checkstyle you
need to run:
